import { FaCheckCircle, FaStar, FaShieldAlt, FaRocket } from 'react-icons/fa';
...
const pricingPlans = [
  ...
  },
  {
    ...
    buttonClass: "bg-white hover:bg-gray-100 text-blue-600",
        ...
        <div className="container mx-auto px-6 py-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 items-stretch">
            {pricingPlans.map((plan, index) => (
              <div
                key={index}
                className={`rounded-xl shadow-lg p-8 flex flex-col justify-between hover:shadow-2xl transition-shadow duration-300 ${plan.bgColor} ${plan.popular ? 'border-4 border-yellow-400 dark:border-yellow-500 transform scale-105' : 'border dark:border-gray-700'}`}
              >
                <div>
                  {plan.popular && (
                    <div className="absolute top-0 -mt-5 left-1/2 transform -translate-x-1/2">
                      <span className="bg-yellow-400 text-gray-800 text-xs font-semibold px-3 py-1 rounded-full uppercase">Popüler</span>
                    </div>
                  )}
                  ...
                  <p className={`text-center text-sm mb-8 ${plan.popular ? 'text-gray-100 dark:text-gray-300' : 'text-gray-600 dark:text-gray-400'}`}>
                    {plan.description}
                  </p>
                  <ul className="space-y-3 mb-8">
                    {plan.features.map((feature, featureIndex) => (
                      <li key={featureIndex} className="flex items-start gap-3">
                        <FaCheckCircle className={`${plan.popular ? 'text-yellow-400' : 'text-blue-500'} flex-shrink-0 mt-1`} />
                        <span className={`${plan.popular ? 'text-gray-100 dark:text-gray-200' : 'text-gray-700 dark:text-gray-300'}`}>{feature}</span>
                      </li>
                    ))}
                  </ul>
                </div>
                <Link
                  href="/#contact" // Or a specific contact page for this plan
          ...
          <div className="mt-20 text-center">
            ...
            <Link
              href="/#contact"
              className="inline-block px-10 py-4 bg-green-500 text-white rounded-lg hover:bg-green-600 transition duration-300 shadow-lg hover:shadow-xl"
            >
              Özel Teklif Alın
            </Link>
          </div>
